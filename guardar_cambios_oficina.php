...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $foto_ofi_nombre = $_POST['foto_actual'] ?? null; // Foto principal actual
    $foto_secundaria_ofi_nombre = $_POST['foto_secundaria_actual'] ?? null; // Foto secundaria actual
    $video_ofi_nombre = $_POST['video_actual'] ?? null; // Video actual
    ...
    // Preparar la consulta SQL para la actualización (incluyendo video_url_ofi)
    ...
    $stmt->bind_param("ssssssddssssssi", 
        ...
        $web_p1_ofi, $web_p2_ofi, $Id_ofi);

    if ($stmt->execute()) {
        $_SESSION['mensaje'] = "✅ Oficina actualizada con éxito. Se han guardado todos los cambios incluyendo multimedia y geolocalización.";
        header("Location: lista_oficinas.php");
        exit();
    } else {
        echo "❌ Error al actualizar la oficina: " . $stmt->error;
    }
