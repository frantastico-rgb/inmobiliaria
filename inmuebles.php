...
<body>
    ...
    <form action="guardar_inmueble.php" method="post" enctype="multipart/form-data">
        ...
        <div>
            <label for="video">Video del Inmueble:</label>
            <input type="file" id="video" name="video" accept="video/*">
            <small style="color: #666; display: block; margin-top: 5px;">
                🎥 Duración máxima: 2 minutos | Tamaño máximo: 50MB | Formatos: MP4, MOV, AVI
            </small>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            try {
                // Crear mapa con vista global inicial
                map = L.map('map-inmueble').setView([4.6097, -74.0817], 2); // Zoom global inicial
                
                // Agregar tiles
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                    attribution: '© OpenStreetMap contributors',
                    maxZoom: 19, // Permitir zoom máximo para detalles
                    minZoom: 1   // Permitir zoom mínimo para vista mundial
                }).addTo(map);
                
                // Después de 2 segundos, centrar en Colombia para uso local
                setTimeout(() => {
                    if (!marker) { // Solo si no hay marcador colocado
            ...
            fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=18&addressdetails=1`)
                ...
                .then(data => {
                    if (data && data.display_name) {
                        ...
                        // Actualizar campos automáticamente si están vacíos
                        ...
                        if (!dirField.value && fullAddress) {
                            dirField.value = fullAddress;
                        }
                        if (!barrioField.value && neighbourhood) {
                            barrioField.value = neighbourhood;
                        }
                        if (!ciudadField.value && city) {
                            ciudadField.value = city;
                        }
                        if (!paisField.value && country) {
                            paisField.value = country;
                        } else if (!paisField.value) {
                            paisField.value = 'Colombia'; // Valor por defecto
                        }
                        ...
                        const popupContent = `
                            <div style="max-width: 250px;">
                                <strong>${countryEmoji} Ubicación encontrada</strong><br>
                                <small>${data.display_name}</small><br><br>
                                <em>💡 Campos actualizados automáticamente</em>
                            </div>
                        `;
                        ...
                        showLocationNotification(`${countryEmoji} Dirección encontrada: ${locationName}, ${country || 'Mundo'}`, 'success');
                        ...
                        `);
                        showLocationNotification('🌍 Coordenadas guardadas', 'info');
                    }
                })
                .catch(error => {
        ...
        function validateVideo(input) {
            ...
            // Validar duración del video (aproximada por tamaño)
            const estimatedMinutes = file.size / (1024 * 1024 * 8); // Estimación muy aproximada
